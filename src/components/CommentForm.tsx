...
function CommentForm({ createComment }: CommentFormProps) {
  ...
  return (
    <>
      <form onSubmit={onCreateComment} className="max-w-md w-full bg-white p-6 rounded-lg border-2 border-gray-300">
        <h1 className="text-base/7 font-semibold text-gray-900">Create Thread</h1>
        <div className="sm:col-span-4 mt-4">
          <label htmlFor="content" className="block text-sm/6 font-medium text-gray-900">Title</label>
          <div className="mt-2">
            <div className="flex items-center rounded-md bg-white pl-3 outline-1 -outline-offset-1 outline-gray-300 focus-within:outline-2 focus-within:-outline-offset-2 focus-within:outline-indigo-600">
              <input type="text" name="content" id="content" value={content} onChange={onContentChangeHandler} className="block min-w-0 grow py-1.5 pr-3 pl-1 text-base text-gray-900 placeholder:text-gray-400 focus:outline-none sm:text-sm/6" placeholder="Content" />
            </div>
          </div>
        </div>
